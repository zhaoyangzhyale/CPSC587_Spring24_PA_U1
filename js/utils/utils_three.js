...
export class ThreeEngine {
    ...
    draw_debug_sphere(center_point, radius, color=0x00ff00, opacity=1.0) {
        ...
        if (this.num_sphere_objects_used_on_curr_frame >= this.sphere_objects.length) {
            for (let i = 0; i < 500; i++) {
                ...
                let sphere_object = spawn_sphere_default(this.scene, undefined, color, opacity, undefined);
...
export function spawn_square_base(scene, width=1, height=1, color=0x0000ff, opacity=1.0, flat_material=true) {
    /*
...
export function spawn_square_default(scene, color=0x0000ff, opacity=1.0, flat_material=true) {
    return spawn_square_base(scene, 1, 1, color, opacity, flat_material);
}

export function spawn_square_specific(scene, center_point, normal_vec, width=1, height=1, color=0x0000ff, opacity=1.0, flat_material=true) {
    let square_object = spawn_square_default(scene, color, opacity, flat_material);
    set_square_specific(square_object, center_point, normal_vec, width, height, color, opacity, flat_material);
    return square_object;
}

export function set_square_specific(square_specific, center_point, normal_vec, width=1, height=1, color=0x0000ff, opacity=1.0, flat_material=true) {
    let a = unroll_matrix_to_list(center_point);

    // let y = convert_z_up_array_to_y_up_array(unroll_matrix_to_list(normal_vec));
    let b = add_matrix_matrix( roll_list_into_matrix(center_point, 3, 1), roll_list_into_matrix(center_point, 3, 1) );
    let bb = convert_z_up_array_to_y_up_array( [b[0][0], b[1][0], b[2][0]] );

    z_up_set_scale(square_specific, width, 0, height);
    // z_up_set_object_rotation_from_quaternion(square_specific, quaternion.w, quaternion.x, quaternion.y, quaternion.z);
    z_up_set_object_position(square_specific, a[0], a[1], a[2]);
    square_specific.lookAt(bb[0], bb[1], bb[2]);
    // square_specific.updateMatrixWorld();
}
